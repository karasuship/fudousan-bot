...
export function diagnoseV2(input: DiagnosisInput2): DiagnosisResult2 {
  const feeIssues =
    input.timing === "pre_contract"
      ? detectPreContractIssues(input.fees, input.preContractContext)
      : detectFeeIssues(input.fees);

  const timelineIssues =
    input.timing === "post_contract"
      ? detectTimelineIssues(input.timeline)
      : [];

  const agentIssues =
    input.timing === "post_contract"
      ? detectAgentResponseIssues(input.agentResponse)
      : [];
...
function getSeverity(
  ...
  if (context?.applicationStatus === "before_apply") return baseSeverity;
  return "medium";
}

export function detectPreContractIssues(
  fees: FeeEntry[],
  context: PreContractContext | undefined
): Issue2[] {
  const issues: Issue2[] = [];
  ...
  for (const fee of fees) {
    ...
    if (fee.feeId === "agency_fee") {
      issues.push({
        id: "pre_agency_check", feeId: fee.feeId, axis: "A", layer: "voluntary",
        severity: getSeverity("high", context), label: `${label}：契約前確認`, strategy: "free_rent",
        axisAText: "仲介手数料は宅建業法上、借主から賃料の0.5ヶ月分が原則です。1ヶ月分とするには原則の説明と書面承諾が必要とされています。他社では0.5ヶ月分や、貸主から受領のため借主0円の場合もあります。",
        axisBText: null, yesNoQuestion: null, evidenceRequest: null,
        explanationRequest: "1ヶ月分とする根拠と、貸主側からの受領の有無をご説明ください。",
      });
    }
    ...
    if (fee.feeId === "key_exchange") {
      issues.push({
        id: "pre_key_check", feeId: fee.feeId, axis: "A", layer: "voluntary",
        severity: getSeverity("high", context), label: `${label}：契約前確認`, strategy: "free_rent",
    ...
    if (fee.feeId === "cleaning") {
      issues.push({
        id: "pre_cleaning_check", feeId: fee.feeId, axis: "A", layer: "voluntary",
        severity: getSeverity("high", context), label: `${label}：契約前確認`, strategy: "free_rent",
    ...
    // ── 消毒・抗菌処理費 ──
    if (fee.feeId === "disinfection") {
      issues.push({
        id: "pre_disinfection_check", feeId: fee.feeId, axis: "A", layer: "voluntary",
        severity: getSeverity("high", context), label: `${label}：契約前確認`, strategy: "delete",
        axisAText: "消毒・抗菌処理は任意サービスです。断っても入居を拒否することはできません。",
        axisBText: null, yesNoQuestion: null, evidenceRequest: null,
        explanationRequest: "今回は不要のため削除をお願いできますでしょうか。",
      });
    }

    // ── 24時間サポート ──
    if (fee.feeId === "support_24h") {
      issues.push({
        id: "pre_support_check", feeId: fee.feeId, axis: "A", layer: "voluntary",
        severity: getSeverity("high", context), label: `${label}：契約前確認`, strategy: "delete",
        axisAText: "24時間サポートは任意サービスです。火災保険の付帯サービスと重複する可能性があります。",
        axisBText: null, yesNoQuestion: null, evidenceRequest: null,
        explanationRequest: "今回は不要のため削除をお願いできますでしょうか。",
      });
    }
    ...
    // ── 書類作成費・事務手数料 ──
    if (fee.feeId === "admin_fee") {
      issues.push({
        id: "pre_admin_check", feeId: fee.feeId, axis: "A", layer: "voluntary",
        severity: getSeverity("high", context), label: `${label}：契約前確認`, strategy: "delete",
        axisAText: "書類作成・事務手続きは宅建業者の基本業務です。仲介手数料と二重請求の可能性があります。",
        axisBText: null, yesNoQuestion: null, evidenceRequest: null,
        explanationRequest: "削除または金額の根拠をご説明ください。",
      });
    }
    ...
    if (fee.feeId === "guarantor") {
      issues.push({
        id: "pre_guarantor_check", feeId: fee.feeId, axis: "A", layer: "source",
        severity: getSeverity("medium", context), label: `${label}：契約前確認`, strategy: "confirm",
        axisAText: "保証会社は複数社から選べる場合があります。また保証会社・管理会社・仲介会社のグループ関係の確認も必要です。",
        axisBText: null, yesNoQuestion: null, evidenceRequest: null,
        explanationRequest: "利用できる保証会社の選択肢と、グループ関係の有無をご説明ください。",
      });
    }
    ...
    if (fee.feeId === "fire_insurance") {
      issues.push({
        id: "pre_fire_check", feeId: fee.feeId, axis: "A", layer: "source",
        severity: getSeverity("medium", context), label: `${label}：契約前確認`, strategy: "free_rent",
        axisAText: "火災保険は本来、借主が保険会社・プランを選べます。貸主が求める最低限の補償内容を満たせば他社プランでも加入可能です。",
        axisBText: null, yesNoQuestion: null, evidenceRequest: null,
        explanationRequest: "貸主が求める最低限の補償内容をご提示ください。自分で保険を選ぶことを検討いたします。",
      });
    }
    ...
    if (fee.feeId === "key_money" && fee.amount != null && fee.amount > 0) {
      issues.push({
        id: "pre_key_money_check", feeId: fee.feeId, axis: "A", layer: "voluntary",
        severity: getSeverity("medium", context), label: `${label}：契約前確認`, strategy: "free_rent",
