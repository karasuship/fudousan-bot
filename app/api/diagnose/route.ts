...
async function generateEmailWithAI(
  ...
  // 任意性説明なし費用の一覧を生成
  const noVoluntaryExplanationFees = input.facts
    .filter((f) => {
      const voluntaryFees = (input as any).voluntaryExplainedFees ?? [];
      return !voluntaryFees.includes(f.realityCategory);
    })
    .filter((f) =>
      ["disinfection", "support_24h", "key_exchange", "admin_fee", "cleaning", "other"].includes(
        f.realityCategory
      )
    );

  const noVoluntarySection =
    noVoluntaryExplanationFees.length > 0
      ? `\n## 任意性の説明がなかった費用（最重要論点）\n${noVoluntaryExplanationFees
          .map(
            (f) =>
              `・${f.perceivedLabel}${f.amount ? `（${f.amount.toLocaleString("ja-JP")}円）` : ""}`
          )
          .join(
            "\n"
          )}\nこれらは「断れます」「任意です」という説明がなかった費用です。同意が成立していない可能性があります。`
      : "";

  const paymentOrderSection = (input as any).paymentBeforeJuusetsu
    ? "\n## 重要：重説前払いあり\n重要事項説明の前に金銭が発生しています。これは手続き上の重大な問題であり、全費用の論点を強化します。"
    : "";

  const combinedFeeSection = (() => {
    const broker = input.feeAmounts?.agency_fee;
    const admin  = input.feeAmounts?.admin_fee;
    const rent   = input.monthlyRent;
    if (!broker || !admin || !rent) return "";
    const combined = broker + admin;
    const cap = Math.round(rent * 1.1);
    if (combined <= cap) return "";
    return `\n## 仲介手数料＋事務手数料の合算超過\n仲介手数料（${broker.toLocaleString("ja-JP")}円）＋事務手数料（${admin.toLocaleString("ja-JP")}円）＝合計${combined.toLocaleString("ja-JP")}円が家賃1ヶ月上限（${cap.toLocaleString("ja-JP")}円）を超えています。`;
  })();
...
## メール作成の優先順位
1. 任意性の説明がなかった費用を最重要論点として先に出す
2. 重説前払いがある場合は手続き違反として全体の論点を強化する
3. 仲介手数料＋事務手数料が合算超過なら上限超過の確認を求める
4. 返金より相殺・フリーレントを先に提案する（合意率が高い）
5. 断定・攻撃的表現を使わない。確認・事実固定・根拠要求を中心にする
